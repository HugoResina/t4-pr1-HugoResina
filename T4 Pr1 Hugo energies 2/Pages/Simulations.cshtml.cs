...
namespace T4_Pr1_Hugo_energies_2.Pages
{
    ...
    {
        public List<ASystem> EnergySystems = new List<ASystem>();
                ...
                using (var reader = new StreamReader(path))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (values[0] == "Solar")
                        {
                            ...
                            solar.Date = DateTime.Parse(values[1]);
                            ...
                            solar.energyGenerated = solar.CalculateEnergy();
                            EnergySystems.Add(solar);
                        }
                        else if (values[0] == "Eolic")
                        {
                            EolicSystem eolic = new EolicSystem();
                            eolic.Date = DateTime.Parse(values[1]);
